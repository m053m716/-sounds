...
%
%  sounds.play(); % default is 'alert' sound
...
%
%  sounds.play('other_sound_file_in_this_folder');
...
%  >> Play 'alert' tone at normal frequency with 50 dB attenuation
...
% Parse path to sound file (must be in ~/+sounds/ folder)
[~,fname,~] = fileparts(soundName);
soundName = fullfile(pname,[fname '.mat']);

if ~exist(soundName,'file')
   warning('No corresponding sound file (%s) in +sounds.',soundName);
   return;
end
